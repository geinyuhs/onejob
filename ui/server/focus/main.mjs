...
const engine=new AgentEngine(codex,new HermesClient({directory,root:join(directory,'hermes-runtime')}));
...
const runtime=new ToolRuntime(store,directory,{search,publicSearch:(query,signal)=>codex.answer({phase:'publicSearch',query,reasoningEffort:store.config()?.reasoningEffort},signal,message=>write({event:'runProgress',message})),emit:write,sync:()=>service.syncFolders()});
...
const service=new ProblemService(store,{chatgpt:engine,claude:new ClaudeClient(join(directory,'claude-workspace'))},{folders,search,tools:runtime,workspace,setup,transcription:new Transcription(new Keychain(process.argv[5])),emit:write});
...
}).on('close',()=>{service.stop();engine.close();Promise.all([runtime.close(),oauth.close()]).finally(()=>{store.close();process.exit(0);});});
process.on('SIGTERM',()=>{service.stop();engine.close();Promise.all([runtime.close(),oauth.close()]).finally(()=>{store.close();process.exit(0);});});
