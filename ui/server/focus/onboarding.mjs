...
export class Onboarding {
  constructor(store) {
    ...
    this.db.exec(`CREATE TABLE IF NOT EXISTS job_flow(problem_id TEXT PRIMARY KEY REFERENCES problems(id),stage TEXT NOT NULL,draft TEXT NOT NULL DEFAULT '',plan TEXT NOT NULL DEFAULT '');
      UPDATE job_flow SET stage='researchReady' WHERE stage='research';
      CREATE TABLE IF NOT EXISTS job_plan_edits(problem_id TEXT PRIMARY KEY REFERENCES problems(id),problem TEXT NOT NULL,research TEXT NOT NULL,source_id TEXT);`);
  }
  state() {const p=this.store.active(),flow=p?(this.db.prepare('SELECT stage,draft,plan FROM job_flow WHERE problem_id=?').get(p.id)||{stage:'work',draft:'',plan:''}):{stage:'empty',draft:'',plan:''};const edit=p&&this.db.prepare('SELECT problem,research FROM job_plan_edits WHERE problem_id=?').get(p.id);return edit?{...flow,planParts:{...edit}}:flow;}
  ...
  finish() {const reply=this.store.snapshot().entries.filter(e=>e.kind==='assistant').at(-1)?.text||'';const id=this.store.requireActive().id;this.db.prepare('DELETE FROM job_plan_edits WHERE problem_id=?').run(id);this.db.prepare("UPDATE job_flow SET plan=?,stage='plan' WHERE problem_id=?").run(reply,id);}
}
