...
export function cleanConnection(input) {
  ...
  if (!['api','mcp','aside','login'].includes(kind)) throw new ProblemError('Choose API, MCP, Aside, or website login.');
  ...
  if (kind==='aside' && input.browserAccessApproved!==true) throw new ProblemError('Connect Aside to allow browser access.');
  ...
  if(kind==='login'&&(!secretRef||!usernameRef||!/^op:\/\/[^/?#\s]+\/[^/?#\s]+\/[^?#\s]+$/u.test(usernameRef)||new URL(url).pathname!=='/'))throw new ProblemError('Use the exact HTTPS login origin and username/password references, never credentials.');
  return {id:randomUUID(),kind,name,url,account,secretRef,...(kind==='login'?{usernameRef}:{}),browserAccessApproved:kind==='aside',enabled:true};
}
export class Connections {
  ...
  public() {return this.all().map(({secretRef,usernameRef,account,...c})=>({...c,hasCredential:!!secretRef||c.auth==='oauth'}));}
