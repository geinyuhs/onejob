...
const cases=[
 ...
 ['problem isolation','store.mjs',"AND problem_id=? AND status!='forgotten' AND kind!='assistant'","AND (problem_id=? OR 1=1) AND status!='forgotten' AND kind!='assistant'",'retrieval stays'],
...
for(const [name,file,before,after,pattern]of cases)test('mutation proof: '+name,t=>{
 ...
 const path=join(target,file),text=readFileSync(path,'utf8');assert.ok(text.includes(before),'mutation still addresses live code');writeFileSync(path,text.replaceAll(before,after));
