...
test('automatic browser setup chooses a single profile and requires the existing privacy approval',async t=>{
 ...
 await assert.rejects(setup.connectAside({privacyConfirmed:false}),/privacy/);await assert.rejects(setup.connectAside({profile:'u9',privacyConfirmed:true}),/available profile/);
 const c=await setup.connectAside({privacyConfirmed:true});assert.equal(c.account,'u0');assert.equal((await setup.connectAside({privacyConfirmed:true})).id,c.id);
 const missing=new ServiceSetup(f.connections,f.oauth,{find:()=>null,exists:()=>false});assert.equal((await missing.inspect()).clients.aside,false);await assert.rejects(missing.connectAside({privacyConfirmed:true}));
});
